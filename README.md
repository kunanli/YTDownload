...
| 微信視頻號 | ❌ [`/sph/` 短連結抓不到，原因見這裡](#為什麼-sph-短連結不可能抓到) |
...
| 微信視頻號「網頁端沒有影片位址」 | [微信沒把影片送到網頁，抓不到](#為什麼-sph-短連結不可能抓到) |
...
微信那條路（但 `/sph/` 短連結[抓不到](#為什麼-sph-短連結不可能抓到)，工具會馬上說明）。
...
> ### 🚫 先講結論：`weixin.qq.com/sph/…` 短連結抓不到
>
> 這不是本工具做不到，是**微信根本沒把影片位址送到網頁端**。詳細證據見
> [下面](#為什麼-sph-短連結不可能抓到)。工具現在會**一秒告訴你**，不會再讓你
> 白等兩分鐘。

微信視頻號的分享連結**沒辦法直接解析** —— 分享頁只是一層 JS 外殼（實測整頁只有
2.5 KB），影片位址要等頁面自己去換才拿得到。

所以這裡走的是另一條路：**先直接問微信的 API，拿得到就下載；拿不到就開一個真正的
瀏覽器把頁面載入，攔截頁面自己發出的請求**。
...
| `--browser` | 即使微信已表明網頁端拿不到，仍然開瀏覽器試一次 |
...
| 頁面呼叫的 `get_feed_info` API | 只回**標題、作者、封面**（`coverUrl`），**沒有 `videoUrl`** |
| 頁面的播放器程式碼 | `pageMode === "sph"` 時**播放被關掉** —— 點下去只會跳出 QR code |
| 改用 `exportId` 問同一支 API | 回 `"此内容暂时无法播放"` |
| 頁面實際發出的 14 個請求 | 最大的「影片 CDN」資源是 46.9 KB 的**封面 jpg** |

所以瀏覽器攔不到影片，是因為**影片位址從來沒被送出來**。等 120 秒、開十次瀏覽器、
先登入再試，結果都一樣。工具現在會直接說明並停止，不再讓你空等。

> 💡 帶 `?eid=` 的連結是另一條路 —— 網頁端在那個模式下**是允許播放的**，工具會
> 自動改走那條。但用這支測試影片的 `eid` 實測，微信回的仍是「此內容暫時無法播放」，
> 所以不保證每支都行。

> ### 唯一實際可行的路
>
> [wechatvideodownload](https://github.com/qiye45/wechatvideodownload) /
> [wx_video_download](https://github.com/qiye45/wx_video_download) —— 攔截**電腦版
> 微信客戶端**的流量並解密。影片本身也是加密的（實測：前 128 KB 被處理過，其餘是
> 明文），這類工具會幫你解開。
>
> 代價是要**安裝根憑證**（等於讓一個程式看得到你所有的加密連線）、代理整台電腦的
> 流量，而且必須開著微信手動播放一次 —— 因此**無法整合進本工具**。
>
...
> ### 瀏覽器這條路還留著嗎？
>
> 留著。API 查不到（網路問題、微信改版）時仍會自動開瀏覽器；`--browser` 可以強制。
> 已驗過的部分：瀏覽器啟動、頁面載入、請求攔截、來源挑選、下載、檔案格式檢查
> （用本機伺服器完整跑過，正確攔到 6.4 MB 的 `video/mp4` 並下載成功）。
>
> 抓到的檔案若不是可播放的影片，工具會**明確告訴你並刪除**，不會留下打不開的檔案。
